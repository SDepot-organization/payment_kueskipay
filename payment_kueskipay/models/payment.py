...
class KueskiPayPaymentAcquirer(models.Model):
	...
	def get_form_action_url(self, tx_values):
		...
		callback_url = self.env['ir.config_parameter'].sudo().get_param('payment.callback_url', '0')
	...
	def render(self, reference, amount, currency_id, partner_id=False, values=None):
		...
		#custom
		payment_status = self.env['ir.config_parameter'].sudo().get_param('payment.status', '0')
		if payment_status != '0':
